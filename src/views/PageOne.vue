<template>
  <div>
    <div style="text-align: center;  font-size: 19px;height: auto; margin-bottom: 20px;">A域文件</div>
...
<script>
import {getAdomain, dlAdata} from '@/api'
export default {
  data() {
    return {
      userForm:{
  ...
  methods: {
    ...
    handleDown(row) {
      this.$confirm('此操作将下载该文件, 是否继续?', '提示', {
        ...
        dlAdata(JSON.stringify(row)).then(() => {
          this.$message({
            type: 'success',
            message: '下载成功!'
          });
    ...
    //获取列表中的数据
      getList(){
        getAdomain({params: {...this.userForm,...this.pageData}}).then(({data}) =>{
          //console.log(data)
          this.tableData2 = data.list
